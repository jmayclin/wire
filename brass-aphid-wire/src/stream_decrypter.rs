...
// thoughts -> we'll have one KeyManager per config, so all of the StreamDecrypter's
// will need to share the KeyManager. And lookup the connection by the client random?

/// The KeyManager holds all of the keys that are generated by the key logging
/// callbacks.
///
/// This is kind of the "temporary home" of keying material, before it is transformed
/// into a keyspace.
#[derive(Debug)]
struct KeyManager {
...
impl KeyManager {
    ...
    fn parse_key_log_line(&mut self, line: &[u8]) {
    ...
    fn handshake_space(&mut self, cipher: iana::Cipher) -> Option<KeySpace> {
    ...
    fn application_space(&mut self, cipher: iana::Cipher) -> Option<KeySpace> {
        let space = KeySpace::new(
...
impl KeySpace {
    fn new(client_secret: &[u8], server_secret: &[u8], cipher: iana::Cipher) -> Self {
        ...
        Self {
            cipher,
            client_secret: client_secret.to_vec(),
...
struct StreamDecrypter {
    ...
    pub key_manager: Box<KeyManager>,
...
impl StreamDecrypter {
    fn new(send: Box<dyn std::io::Write>, read: Box<dyn std::io::Read>) -> Self {
        Self {
            identity: None,
            ...
            key_manager: Box::new(KeyManager::new()),
            current_space: None,
                ...
                record_buffer = match record_header.content_type {
                    ...
                    ContentType::ApplicationData => {
                            ...
                            && mode == Mode::Client
                        {
                            self.current_space = self
                                .key_manager
                                .application_space(self.selected_cipher.unwrap());
                        }
                        self.handshake_message.push((mode, handshake_message));
...
mod s2n_tls_decryption {
    ...
    fn s2n_server_test() -> anyhow::Result<()> {
        ...
        let key_manager = Box::new(KeyManager::new());
        ...
        unsafe {
            server_config.set_key_log_callback(
                ...
                key_manager.as_ref() as *const KeyManager as *mut c_void,
            )?
        };
        let mut test_pair =
